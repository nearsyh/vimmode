/*
 * Breach [mod_strip] vim_keyboard.js
 *
...
// @spec { }
// ```
var vim_keyboard = function(spec, my) {
  var _super = {};
  my = my || {};
  spec = spec || {};
  ...
  var vim_shortcut_enter_normal; /* vim_shortcut_enter_normal(); */
  var vim_shortcut_enter_insert; /* vim_shortcut_enter_insert(); */
  var handler_normal;            /* handler_normal(event); */
  var handler_insert;            /* handler_insert(event); */

  // ## key to message
  var key2msg = {
    84 : 'new',   88 : 'close',   82 : 'reload',
    74 : 'down',  75 : 'up',  72 : 'left',  76 : 'right',
  };
  ...
  /* PRIVATE HELPERS */
  /****************************************************************************/
  vim_shortcut_enter_normal = function() {
    my.mode = my.MODE_NORMAL;
  }

  vim_shortcut_enter_insert = function() {
    my.mode = my.MODE_INSERT;
  }
  ...
  handler = function(evt) {
    ...
    console.log("[vim_mode] " + evt.keycode);
  ...
  handler_normal = function(evt) {
    if(evt.type !== 7) return;

    that.emit(key2msg[evt.keycode]);
    /*
    if(evt.keycode === 84) {
      that.emit(key2msg[evt.keycode]);
  ...
  handler_backup = function(evt) {
    ...
    if(evt.type === 7 && (evt.modifiers === modifier) &&
       evt.keycode === 219 && !is_last(evt)) {
      /* Ctrl - [ ; No Repetition */
      that.emit('back');
    }
    if(evt.type === 7 && (evt.modifiers === modifier) &&
       evt.keycode === 221 && !is_last(evt)) {
      /* Ctrl - ] ; No Repetition */
      that.emit('forward');
    }
  ...
  handler_insert = function(evt) {
    if(evt.type !== 7) return;

    // Escape
    if(evt.keycode === 27) {
      vim_shortcut_enter_normal();
    }
  }
...
exports.vim_keyboard = vim_keyboard;
