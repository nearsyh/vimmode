...
var vim_handler = function(spec, my) {
  ...
  vim_shortcut_close = function() {
    console.log('[vimmode] close tab');
    breach.module('core').call('tabs_current', {}, function(err, res) {
      return breach.module('core').call('tabs_close', {
        id: res.id,
      }, function(err) {
        if(err) return console.log('Unexpected Error : %s', err);
        else return console.log('[vimmode] tab %id closed', res);
      });
    });
  }

  vim_shortcut_reload = function() {
    console.log('[vimmode] reload');
    breach.module('core').call('tabs_current', {}, function(err, res) {
      return breach.module('core').call('tabs_reload', {
        id: res.id,
      }, function(err) {
        if(err) return console.log('Unexpected Error : %s', err);
        else return console.log('[vimmode] tab %id reloaded', res);
      });
    });
  }
